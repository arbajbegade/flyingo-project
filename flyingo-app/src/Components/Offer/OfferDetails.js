...
function OfferDetails() {
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-16">
      <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        <div className="text-2xl font-semibold mb-4 text-gray-800">{offer.title}</div>
        ...
        <div className="bg-gray-200 p-4 rounded-lg mb-4">
          ...
          <div className="flex justify-center items-center mt-2 space-x-4">
            ...
            <span className="text-blue-500 font-semibold">{offer.discount}</span>
            <span className="text-gray-500">{offer.validity}</span>
          </div>
        </div>

        <div>
          <div className="text-lg font-semibold mb-2 text-gray-700">About this offer:</div>
          <ul className="list-disc pl-6 space-y-2">
            {offer.about.map((x, index) => (
              <li key={index} className="text-gray-600">{x}</li>
            ))}
          </ul>
        </div>
      </div>
